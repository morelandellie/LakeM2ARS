      ...
      implicit none
      include 'mars_ice.inc' ! info for simulation
...
!      output filesA05_all_mars_inputs
      open(unit=50,file='output/cold_w5_Tprof.dat',status='unknown')
      open(unit=51,file='output/cold_w5_surf.dat',status='unknown')
...
!c~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      real tempice,hice,hsnow,fracice,surf_a,evap,xtime,day, &
           ...
           o18snow,deutsnow ! SD DELETED qew,qhw,sww,luw
      logical snow_flag,melt_flag
      ...
      if (i_shuf.eq.1) then  ! get info from common block
        ...
        true_depth = depth + d_frac   !EM added
...
!SD                         evap_ave*econv/nsteps,mixmax,depth
           write(51,391) day,tsurf_ave/nsteps,mix_ave/nsteps,&          !SD HEAT BUDGET
                         ...
                         luw_ave/nsteps,hice_ave/nsteps, &
                         fice_ave/nsteps,mixmax,depth,d_frac,true_depth
           write(50,392) day,((temp_ave(k)/nsteps),k=1,depth)
  391      format(10(F20.4,1x),2(i3,1x),2(F20.4,1x))     !Yueyang modified this line to add two ice.
  392      format(58(F20.2,1x)) !Here change the temperature profile format.
           end if
